...
export class MetadataComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['id', 'createdBy', 'updatedBy', 'action', 'source'];

  subscription: Subscription | null = null;
  metadata = signal<IMetadata[]>([]);
  isLoading = false;
  ...
  trackId = (item: IMetadata): string => this.metadataService.getMetadataIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && MetadataComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && MetadataComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.metadata.set(this.refineData(dataFromBody));
  }
